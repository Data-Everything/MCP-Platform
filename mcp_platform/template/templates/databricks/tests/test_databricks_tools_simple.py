...
class TestDatabricksToolValidation:
    ...
    def test_readonly_mode_restrictions(self):
        ...
        # These would be blocked by read-only mode if implemented:
        # CREATE, INSERT, UPDATE, DELETE, DROP, ALTER, TRUNCATE, MERGE, COPY, etc.
        ...
        for tool in tools:
            assert (
                tool["name"] in read_tools
            ), f"Tool {tool['name']} should be read-only safe"
    ...
    def test_tool_naming_conventions(self):
        ...
        for tool in tools:
            ...
            assert (
                "_" in tool_name or tool_name.islower()
            ), f"Tool {tool_name} should use snake_case"
            ...
            assert (
                len(description) > 10
            ), f"Tool {tool_name} description should be meaningful"
    ...
    def test_security_configuration_validation(self):
        ...
        assert (
            read_only_config.get("default") is True
        ...
        for field in sensitive_fields:
            if field in properties:
                assert (
                    properties[field].get("sensitive") is True
                ), f"Field {field} should be marked as sensitive"
    ...
    def test_filtering_configuration_presence(self):
        ...
        assert (
            "allowed_databases" in properties
        ), "Should have allowed_databases configuration"
        assert (
            "allowed_schemas" in properties
        ), "Should have allowed_schemas configuration"

        # Check defaults allow all access
        assert (
            properties["allowed_databases"].get("default") == "*"
        ), "Should default to allowing all databases"
        assert (
            properties["allowed_schemas"].get("default") == "*"
        ), "Should default to allowing all schemas"
    ...
    def test_capabilities_documentation(self):
        ...
        assert (
            len(capabilities) >= 4
        ), f"Expected at least 4 capabilities, found {len(capabilities)}"
        ...
        for expected_cap in expected_capability_names:
            ...
            ), f"Missing capability: {expected_cap}"
