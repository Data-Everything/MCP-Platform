...
try:
    ...
    logger.error(
        "Failed to import Databricks SDK. Install with: pip install databricks-sdk"
    )
    raise ImportError(
...
class DatabricksMCPServer:
    ...
    def _filter_schemas(self, schemas: List[str]) -> List[str]:
        ...
        return [
            ...
            if self._match_pattern(schema, allowed_patterns)
        ]
                ...
                cluster_list.append(
                    {
                        ...
                        "runtime_engine": (
                            cluster.runtime_engine.value
                            if cluster.runtime_engine
                            else None
                        ),
                ...
                warehouse_list.append(
                    {
                        ...
                        "state": (
                            warehouse.state.value if warehouse.state else "unknown"
                        ),
    ...
    async def list_databases(self, pattern: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for catalog in catalogs:
                ...
                try:
                    schemas = list(
                        self.workspace_client.schemas.list(catalog_name=catalog_name)
                    )
                    ...
                    self.logger.warning(
                        "Failed to list schemas for catalog %s: %s",
                        catalog_name,
                        str(e),
                    )

            # Apply access filtering
            filtered_databases = self._filter_databases(
                [db["database"] for db in databases]
            )

            # Filter the database list to only include allowed ones
            filtered_db_list = [
                db for db in databases if db["database"] in filtered_databases
            ]

            return {"databases": filtered_db_list, "count": len(filtered_db_list)}
    ...
    async def list_tables(
        ...
        try:
            ...
            if not self._match_pattern(
                full_database_name, self.config_data.get("allowed_databases", "*")
            ):
                return {
                    "error": f"Access to database '{full_database_name}' is not allowed"
                }

            # Check if schema is allowed
            if not self._match_pattern(
                schema_name, self.config_data.get("allowed_schemas", "*")
            ):
                ...
                table_list.append(
                    {
                        ...
                        "table_type": (
                            table.table_type.value if table.table_type else None
                        ),
                        "data_source_format": (
    ...
    async def describe_table(
        ...
        try:
                    ...
                    columns.append(
                        {
                            "name": column.name,
                            "type_name": (
                                column.type_name.value if column.type_name else None
                            ),
            ...
            return {
                ...
                "table_type": (
                    table_info.table_type.value if table_info.table_type else None
                ),
                "data_source_format": (
    ...
    async def execute_query(
        ...
        try:
            ...
            response = self.workspace_client.statement_execution.execute_statement(
                statement=query, warehouse_id=warehouse_id, wait_timeout="10m"
            )
            ...
            if (
                ...
                for col in response.manifest.schema.columns:
                    columns.append({"name": col.name, "type": col.type_name})
    ...
    async def get_cluster_info(self, cluster_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "spark_version": cluster.spark_version,
                "runtime_engine": (
                    cluster.runtime_engine.value if cluster.runtime_engine else None
                ),
                "creator_user_name": cluster.creator_user_name,
                "start_time": cluster.start_time,
                "default_tags": (
                    dict(cluster.default_tags) if cluster.default_tags else {}
                ),
    ...
    async def get_warehouse_info(self, warehouse_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "warehouse_type": (
                    warehouse.warehouse_type.value if warehouse.warehouse_type else None
                ),
                ...
                "spot_instance_policy": (
                    warehouse.spot_instance_policy.value
...
if __name__ == "__main__":
    # Create the server instance only when running as main
