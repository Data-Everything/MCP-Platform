...
class TestSnowflakeMCPServer:
    ...
    def mock_snowflake_connector(self):
        ...
        # Mock the snowflake module first
        mock_snowflake = Mock()
        mock_connector = Mock()
    ...
    def test_invalid_filter_pattern(self, mock_snowflake_connector, basic_config):
        ...
        mock_connector, mock_connection, mock_cursor = mock_snowflake_connector
        config = {**basic_config, "database_filter_pattern": "["}  # Invalid regex
    ...
    def test_connection_creation_username_password(
        self, mock_snowflake_connector, basic_config
    ):
    ...
    def test_connection_creation_jwt(self, mock_snowflake_connector):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch.object(
                SnowflakeMCPServer, "_load_private_key", return_value=mock_private_key
            ):
                server = SnowflakeMCPServer(config_dict=config)
                server._create_connection()
    ...
    def test_read_only_query_validation(self, mock_snowflake_connector, basic_config):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            for query in prohibited_queries:
                with pytest.raises(
                    ValueError, match="Query not allowed in read-only mode"
                ):
                    server._validate_read_only_query(query)

    def test_read_only_disabled_allows_all_queries(
        self, mock_snowflake_connector, basic_config
    ):
    ...
    def test_list_schemas_database_filter_check(
        self, mock_snowflake_connector, basic_config
    ):
    ...
    def test_execute_query_with_limit(self, mock_snowflake_connector, basic_config):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            server.execute_query("SELECT * FROM users", limit=50)
    ...
    def test_execute_query_read_only_violation(
        self, mock_snowflake_connector, basic_config
    ):
